...
export function ReportsList() {
  ...
  const viewReport = async (report: DailyReport) => {
    ...
    try {
      const { data: tempLogs, error: tempError } = await supabase
        ...
        .select(`
          *,
          temperature_items (
            name_no,
            temperature_zones (name_no, min_temp, max_temp)
          ),
          profiles (full_name)
        `)
        .eq('recorded_date', report.report_date)
      ...
      const { data: cleaningLogs, error: cleaningError } = await supabase
        ...
        .select(`
          *,
          cleaning_tasks (name_no, frequency),
          profiles (full_name)
        `)
  ...
  const deleteReport = async (id: string) => {
    ...
    try {
      await supabase.from('temperature_logs').delete().eq('recorded_date', reports.find(r => r.id === id)?.report_date || '');
  ...
  const deleteBulkReports = async () => {
    ...
    try {
      for (const id of selectedReports) {
        ...
        if (report) {
          await supabase.from('temperature_logs').delete().eq('recorded_date', report.report_date);
  ...
  const editReport = async (report: DailyReport) => {
    ...
    try {
      ...
      const { data: cleaningLogs } = await supabase
        ...
        .select(`
          *,
          cleaning_tasks (name_no, frequency, description),
          profiles (full_name)
        `)
        .eq('log_date', report.report_date)
        .order('completed_at', { ascending: true });
